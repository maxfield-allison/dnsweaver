...
type mockProvider struct {
	...
	createErr  error
	deleteErr  error
	listResult []provider.Record
	listErr    error

	createCalls []provider.Record
	deleteCalls []provider.Record
}
...
func (m *mockProvider) List(ctx context.Context) ([]provider.Record, error) {
	if m.listErr != nil {
...
func (m *mockProvider) Create(ctx context.Context, record provider.Record) error {
	m.createCalls = append(m.createCalls, record)
	return m.createErr
}

func (m *mockProvider) Delete(ctx context.Context, record provider.Record) error {
	m.deleteCalls = append(m.deleteCalls, record)
	return m.deleteErr
}
...
func (m *mockSource) Extract(ctx context.Context, labels map[string]string) ([]source.Hostname, error) {
	if m.extractErr != nil {
